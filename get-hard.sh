...
function crypto_packages() {
  ...
  while :; do
    ...
    if [[ ${INSTALLCRYPTO,,} == "y" || ${INSTALLCRYPTO,,} == "Y" || ${INSTALLCRYPTO,,} == "N" || ${INSTALLCRYPTO,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}"
  ...
  if [ "${INSTALLCRYPTO,,}" = "Y" ] || [ "${INSTALLCRYPTO,,}" = "y" ]; then
    echo -e "\n"
    echo -e -n "${yellow}"
...
function add_user() {
  ...
  while :; do
    ...
    if [[ ${ADDUSER,,} == "y" || ${ADDUSER,,} == "Y" || ${ADDUSER,,} == "N" || ${ADDUSER,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}"
  ...
  if [ "${ADDUSER,,}" = "Y" ] || [ "${ADDUSER,,}" = "y" ]; then
    ...
    while [[ "$UNAME" =~ [^0-9A-Za-z]+ ]] || [ -z "$UNAME" ]; do
      echo -e "\n"
      echo -e -n "${lightred}"
...
function collect_sshd() {
  ...
  while :; do
    ...
    [[ $SSHPORT =~ ^[0-9]+$ ]] || {
      echo -e -n "${lightred}"
      echo -e " --> Try harder, that's not even a number. \n"
      echo -e -n "${nocolor}"
      continue
    }
...
function prompt_rootlogin {
  ...
  if [ -n "${UNAME,,}" ]; then
    ...
    while :; do
      ...
      if [[ ${ROOTLOGIN,,} == "y" || ${ROOTLOGIN,,} == "Y" || ${ROOTLOGIN,,} == "N" || ${ROOTLOGIN,,} == "n" ]]; then
        break
      fi
    done
    echo -e "${nocolor}"
    ...
    if [ "${ROOTLOGIN,,}" = "Y" ] || [ "${ROOTLOGIN,,}" = "y" ]; then
      :
...
function disable_passauth() {
  ...
  if [ -f "/root/.ssh/authorized_keys" ]; then
    ...
    while :; do
      ...
      if [[ ${PASSLOGIN,,} == "y" || ${PASSLOGIN,,} == "Y" || ${PASSLOGIN,,} == "N" || ${PASSLOGIN,,} == "n" ]]; then
        break
      fi
    done
    echo -e "${nocolor}\n"
    ...
    if [ "${PASSLOGIN,,}" = "Y" ] || [ "${PASSLOGIN,,}" = "y" ]; then
...
function ufw_config() {
  ...
  while :; do
    ...
    if [[ ${FIREWALLP,,} == "y" || ${FIREWALLP,,} == "Y" || ${FIREWALLP,,} == "N" || ${FIREWALLP,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}\n"

  if [ "${FIREWALLP,,}" = "Y" ] || [ "${FIREWALLP,,}" = "y" ]; then
    echo -e -n "${nocolor}"
...
function server_hardening() {
  ...
  while :; do
    ...
    if [[ ${GETHARD,,} == "y" || ${GETHARD,,} == "Y" || ${GETHARD,,} == "N" || ${GETHARD,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}\n"
...
function google_auth() {
  ...
  while :; do
    ...
    if [[ ${GOOGLEAUTH,,} == "y" || ${GOOGLEAUTH,,} == "Y" || ${GOOGLEAUTH,,} == "N" || ${GOOGLEAUTH,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}\n"
  ...
  if [ "${GOOGLEAUTH,,}" = "Y" ] || [ "${GOOGLEAUTH,,}" = "y" ]; then
    ...
    if [ "${UNAME,,}" ] && [ -e /root/.google_authenticator ]; then # copy root Google Authenticator file new non-root user
  ...
  if [[ -r /etc/os-release ]]; then
    ...
    if [[ "${VERSION_ID}" != "16.04" ]]; then
      ...
      while :; do
        ...
        if [[ ${KSPLICE,,} == "y" || ${KSPLICE,,} == "Y" || ${KSPLICE,,} == "N" || ${KSPLICE,,} == "n" ]]; then
          break
        fi
      done
      echo -e "${nocolor}\n"

      if [ "${KSPLICE,,}" = "Y" ] || [ "${KSPLICE,,}" = "y" ]; then
        ...
        if [ -e /etc/uptrack/uptrack.conf ]; then
          echo -e -n "${lightgreen}"
          echo -e "---------------------------------------------------- " | tee -a "$LOGFILE"
...
function motd_install() {
  ...
  while :; do
    ...
    if [[ ${MOTDP,,} == "y" || ${MOTDP,,} == "Y" || ${MOTDP,,} == "N" || ${MOTDP,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}\n"
  ...
  if [ "${MOTDP,,}" = "Y" ] || [ "${MOTDP,,}" = "y" ]; then
    ...
    touch /etc/update-motd.d/99-esm
...
function restart_sshd() {
  ...
  while :; do
    ...
    if [[ ${SSHDRESTART,,} == "y" || ${SSHDRESTART,,} == "Y" || ${SSHDRESTART,,} == "N" || ${SSHDRESTART,,} == "n" ]]; then
      break
    fi
  done
  echo -e "${nocolor}\n"
  ...
  if [ "${SSHDRESTART,,}" = "Y" ] || [ "${SSHDRESTART,,}" = "y" ]; then
    # insert a pause or delay to add suspense
    systemctl restart sshd
    if [ "$FIREWALLP" = "yes" ] || [ "$FIREWALLP" = "y" ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      echo -e -n "${nocolor}"
      if
        [ "$FIREWALLP" = "yes" ] || [ "$FIREWALLP" = "y" ]
        echo -e -n "${lightgreen}"
      then
        echo " $(date +%m.%d.%Y_%H:%M:%S) : SUCCESS : UFW firewall enabled" | tee -a "$LOGFILE"
        echo -e "------------------------------------------------------ " | tee -a "$LOGFILE"
        echo -e -n "${nocolor}"
      else
        :
...
function install_complete() {
  ...
  if [ "${INSTALLCRYPTO,,}" = "yes" ] || [ "${INSTALLCRYPTO,,}" = "y" ]; then
    echo -e " --> Common crypto packages were installed" | tee -a "$LOGFILE"
  fi
  if [ -n "${UNAME,,}" ]; then
    echo -e "${white} We created a non-root user named (lower case):${nocolor}" "${UNAME,,}" | tee -a "$LOGFILE"
  else
    echo -e "${white} A new user was not created during the setup process ${nocolor}" | tee -a "$LOGFILE"
  fi
  echo " ${white}PasswordAuthentication settings:${lightred}" "$PASSWDAUTH" | tee -a "$LOGFILE"
  if [ "${FIREWALLP,,}" = "yes" ] || [ "${FIREWALLP,,}" = "y" ]; then
    echo -e "${lightcyan} --> UFW was installed and basic firewall rules were added" | tee -a "$LOGFILE"
  else
    echo -e "${lightcyan} --> UFW was not installed or configured" | tee -a "$LOGFILE"
  fi
  ...
  # fi
  if [ "${KSPLICE,,}" = "yes" ] || [ "${KSPLICE,,}" = "y" ]; then
    echo -e " You installed Oracle's Ksplice to update without reboot" | tee -a "$LOGFILE"
  else
    echo -e " You chose NOT to auto-update OS with Oracle's Ksplice" | tee -a "$LOGFILE"
  fi
  echo -e "${yellow}-------------------------------------------------------- " | tee -a "$LOGFILE"
